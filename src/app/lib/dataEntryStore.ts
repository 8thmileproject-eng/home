...
export async function getAllDataEntries(projectId?: string): Promise<DataEntry[]> {
...
export async function insertDataEntry(entry: Omit<DataEntry, "_id">): Promise<string | null> {
  const db = await getDb();
...
export async function updateDataEntry(id: string, updates: Partial<Omit<DataEntry, "_id">>): Promise<boolean> {
  const db = await getDb();
