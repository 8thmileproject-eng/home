...
function TypeEditor({ value, onChange, placeholder, onSwitchToDraw }: {
  value: string;
  ...
  const editor = useEditor({
    ...
    content: value,
    editorProps: {
      attributes: { class: "prose prose-sm max-w-none focus:outline-none min-h-[100px] px-4 py-3 text-gray-900" },
    },
    onUpdate: ({ editor }) => {
...
export default function NotepadInput({ value, onChange, placeholder }: NotepadInputProps) {
  ...
  const switchToDraw = () => { onChange(`<img src="" data-drawing="true" />`); };
  ...
  const switchToText = useCallback(async () => {
    ...
    if (canvas?.hasStrokes()) {
      ...
      if (text) { onChange(text); setEditorKey((k) => k + 1); } else { onChange(""); }
  ...
  const handleDrawingChange = (val: string) => {
    ...
    } else { onChange(val); setEditorKey((k) => k + 1); }
  };
